...
\begin{figure}[htbp]
  ...
  \caption{%
    \emph{Self-announcement causal chain.} Single-canvas,four-row stacked view of fault~1 of the V4 schedule
    ($t_1{=}\SI{12}{\second}$, drone~0 severed). Rows, top to bottom: surviving rope tension $T_1(t)$; feed-forward command $T_1^{\mathrm{ff}}(t)$; commanded thrust $f_1(t)$; payload altitude error $e_{L,z}(t)$. Teal solid: baseline (FF-on); orange dashed: P2-A ablation (FF-off, identity disabled). At fault, the surviving rope's measured tension rises immediately (row~1) regardless of controller configuration. With the identity active, $T_1^{\mathrm{ff}}$ tracks $T_1$ at the next control tick (row~2), commanded thrust steps proportionally (row~3), and the altitude error is bounded (row~4). With the identity disabled, $T_1^{\mathrm{ff}} \equiv 0$, the thrust step is absent, and the altitude error grows by $\sim\!300$~mm before the slow PD recovers --- visually closing the causal chain \emph{measured tension} $\to$ \emph{feed-forward} $\to$ \emph{thrust step} $\to$ \emph{altitude recovery}.%
  }
...
\paragraph{P2-D --- Formation reshape.} We sweep the reference lemniscate period $T_{\mathrm{ref}} \in \{8,\,10,\,12\}~\si{\second}$ with and without the post-fault reshape supervisor, on the V4 dual-fault schedule. Results are shown in \cref{tab:reshape-period-sweep}.
...
\label{rem:reshape-null}
  The post-fault reshape supervisor assigns surviving drones to the tension-optimal angular-slot configuration for quasi-static hover (\cref{sec:reshape-supervisor}). At the tested lemniscate periods, the centripetal acceleration during cruise is $a_c = (2\pi/T_{\mathrm{ref}})^2 A \approx 0.82$--$3.29~\si{\metre\per\square\second}$ across $T_{\mathrm{ref}} \in \{6,8,10,12\}~\si{\second}$, which continuously modulates the rope-tension distribution throughout the trajectory and dominates the hover-equilibrium asymmetry that the reshape supervisor targets. The quasi-static-hover assumption underlying the closed-form slot assignment of \cref{thm:reshape} is therefore not the binding constraint at any of the tested periods --- including the aggressive $T_{\mathrm{ref}}=\SI{6}{\second}$ probe (E8), which was a priori predicted to yield an approximately $25.8\,\%$ peak tension reduction. The measured peak tension is $330.2~\si{\newton}$ with reshape on and $330.2~\si{\newton}$ with reshape off (\cref{fig:reshape-binding-T6}, \cref{tab:reshape-period-sweep}): $0.0\,\%$ measurable benefit. The null finding at $T_{\mathrm{ref}}=\SI{6}{\second}$ indicates that the dynamic centripetal load dominates the quasi-static slot-assignment target even when the trajectory is driven towards the linearised theorem's binding regime, and that pushing the reshape layer into an empirically binding regime would require either a lower-bandwidth reference family or an extended reshape objective that accounts for the dynamic load. Theorem~\ref{thm:reshape} and \cref{sec:theorem-preservation} guarantee that enabling the reshape outside its binding regime leaves the recovery envelope of \cref{thm:hybrid-stability} unchanged.
\end{remark}
...
\begin{table}[htbp]
  \caption{%
    Formation-reshape period sweep (P2-D, NF3). Peak post-fault rope tension (N) with and without the reshape supervisor across three trajectory periods on the V4 schedule; the aggressive-period binding-regime probe at $T_{\mathrm{ref}}=\SI{6}{\second}$ (E8) is reported alongside in \cref{fig:reshape-binding-T6}. During the tested periods, the peaks are trajectory-dominated, leaving reshape without measurable effect on the peak metric.%
  }
...
A reviewer's concern with cascade architectures is that the controller may quietly rely on actuator saturation to absorb post-fault loads. We audit this by reporting the per-drone commanded thrust ratio $f_i(t)/f_{\max}$ across the $[8,30]\,\si{\second}$ cruise window for V3, V4, V5, V6 (\cref{fig:actuator-margin}) and the per-variant maxima and above-90\,\% saturation fractions (\cref{tab:actuator-margin}). The peak commanded ratio is $0.73$ on V3--V5 (drone 4 immediately after fault~1) and $0.79$ on V6, with \emph{zero percent} of cruise-window time spent above $0.9\,f_{\max}$ on any variant. The recovery is therefore not actuator-limited.
...
\begin{table}[htbp]
  \caption{%
    Actuator-margin audit. Maximum per-drone commanded thrust ratio over the $[8,30]\,\si{\second}$ cruise window, the drone attaining that maximum, and the fraction of cruise time spent above $0.9\,f_{\max}$.%
  }
