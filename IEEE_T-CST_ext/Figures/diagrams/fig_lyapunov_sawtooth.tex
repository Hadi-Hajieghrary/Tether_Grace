% ============================================================
%  Figure — Fault timeline (top), survivor cardinality decrement,
%  and Lyapunov sawtooth (bottom).  On-figure labels are SHORT
%  ("flow", "jump", "contraction") and the math expressions and
%  lemma references are deferred to the caption.  This keeps
%  every label legible and never overlapping the plot curves.
% ============================================================
\begin{figure}[t]
\centering
\begin{tikzpicture}[
  ...
  \begin{scope}[yshift=2.6cm]
    ...
    \node[tick, anchor=east] at (-0.1, 1.5) {$N{=}5$};
    \node[tick, anchor=east] at (-0.1, 1.0) {$4$};
    \node[tick, anchor=east] at (-0.1, 0.5) {$3$};
  ...
  \draw[decorate, decoration={brace, amplitude=2pt, mirror},
        thin] (2.0, -0.6) -- (5.0, -0.6)
        node[annot, midway, below=2pt]
...
decay of Lemma~\ref{lem:altitude-iss} gives
...
(Lemma~\ref{lem:jump-continuity}). \textbf{Contraction:} composing
...
(Lemma~\ref{lem:dwell-decay}). The H2 hypothesis
