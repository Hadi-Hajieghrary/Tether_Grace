...
\date{Final campaign: 2026-04-21}
...
companion repository. The document was written against the code state
at campaign 2026-04-21 (git tree at the top-level commit).
...
Research/
 ...
 |- run_ieee_campaign.sh        # one-shot campaign runner
 |- docs/                       # all documentation (including this one)
 |- archive_obsolete_designs/   # in-repo provenance of earlier iterations
\end{lstlisting}
...
\chapter{Extension Ideas for a Student Project}
\label{chap:ext}

\begin{enumerate}
\item \emph{Adaptive outer loop} ($L_1$ or MRAC) to track
uncertain payload mass and rope stiffness online.
\item \emph{Receding-horizon MPC} replacing the single-step QP,
including a hard tension-ceiling constraint.
\item \emph{Formation re-shaping} after a fault: have the survivors
relocate to minimise $\max_j T_j$. Requires knowing their positions,
so it breaks the fully-local property cleanly.
\item \emph{Decentralised payload-mass estimation} from a drone's own
$T_i$ history, reconciled with peers via a consensus round.
