/// @file decentralized_mpc_two_drones_test_main.cc
///
/// Phase 2 Test: Two Quadcopters with Decentralized Optimal Control
///
/// Tests implicit coordination: two independent decentralized controllers
/// share the same reference trajectory but do not communicate directly.
/// Expected behavior:
/// - Both drones hover at z=2.0m, load settles below
/// - Equal thrust sharing: T_0 ≈ T_1 ≈ m_load*g/2
/// - Load tracks reference without external coordination signal
/// - Optional: cable severance at user-specified time
...
int main(int argc, char* argv[]) {
  ...
  int headless = 0;
  double severance_time_0 = -1.0;  // -1 = no severance
  ...
  std::cout << "=== Decentralized MPC Phase 2 Test (Two Drones) ===\n";
  ...
  std::cout << "Drone 1 cable severance: "
            << (severance_time_1 >= 0 ? std::to_string(severance_time_1) + " s"
                                        : "none")
            << "\n";
  std::cout << "Visualization: " << (headless ? "disabled" : "enabled") << "\n\n";
  ...
  // ============ ADD ROPES (ONE FOR EACH DRONE) ============

  // Rope 0 (Quad 0 to Payload)
  ...
  // Note: Both ropes apply forces to the same plant
  // Drake doesn't allow multiple connections to the same input port,
  // so we need to combine the forces first (for Phase 2.5 enhancement)
  // For now, only connect rope 0
  builder.Connect(plant->get_state_output_port(),
                  rope_forces_0->get_plant_state_input_port());
  builder.Connect(rope_forces_0->get_forces_output_port(),
                  plant->get_applied_spatial_force_input_port());

  // TODO Phase 2.5: Add force multiplexer to combine rope_forces_0 and rope_forces_1
  // For now, rope_forces_1 computes forces but they are not applied
  builder.Connect(plant->get_state_output_port(),
  ...
  // Connect shared references to both controllers
  ...
  // Controller 0 gets drone 0 state
  builder.Connect(state_splitter->get_output_port(0),  // p_drone0
                  controller_0->get_drone_position_input_port());
  builder.Connect(state_splitter->get_output_port(1),  // v_drone0
                  controller_0->get_drone_velocity_input_port());

  // Controller 1 gets drone 1 state
  builder.Connect(state_splitter->get_output_port(4),  // p_drone1
                  controller_1->get_drone_position_input_port());
  builder.Connect(state_splitter->get_output_port(5),  // v_drone1
                  controller_1->get_drone_velocity_input_port());

  // ============ CABLE TENSION & DIRECTION ============

  // Cable severance injectors (optional)
  ...
  // Connect splitter cable measurements through severance injectors
  builder.Connect(state_splitter->get_output_port(2),  // T_cable_0
                  severance_0->get_input_port(0));
  builder.Connect(severance_0->get_output_port(0),
                  controller_0->get_cable_tension_input_port());
  builder.Connect(state_splitter->get_output_port(3),  // n_cable_0
                  controller_0->get_cable_direction_input_port());

  builder.Connect(state_splitter->get_output_port(6),  // T_cable_1
                  severance_1->get_input_port(0));
  builder.Connect(severance_1->get_output_port(0),
                  controller_1->get_cable_tension_input_port());
  builder.Connect(state_splitter->get_output_port(7),  // n_cable_1
                  controller_1->get_cable_direction_input_port());

  // ============ IMPLICIT COORDINATION ============
  // Each controller knows T_others by receiving from the other drone
  // For simplicity, we assume both controllers are informed of the other's tension

  // Create constant sources for other drone tensions (will be updated in Phase 2.5)
  auto* T_others_0 = builder.AddSystem<drake::systems::ConstantVectorSource<double>>(
      Eigen::Vector4d(0, 0, 0, 0));
  auto* T_others_1 = builder.AddSystem<drake::systems::ConstantVectorSource<double>>(
      Eigen::Vector4d(0, 0, 0, 0));

  builder.Connect(T_others_0->get_output_port(),
                  controller_0->get_other_tensions_input_port());
  builder.Connect(T_others_1->get_output_port(),
  ...
  // Quad 1 at [2, 0, 1.5] (offset for visualization)
  ...
  std::cout << "\nSimulation results:\n";
  std::cout << "  Simulation time: " << simulation_duration << " s\n";
  std::cout << "  Real-time factor: 0.1x\n";
