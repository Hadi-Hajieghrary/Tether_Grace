/// @file main.cc
/// Drake simulation: Multiple quadcopters lift a payload using flexible
/// ropes/tethers.
///
/// === Physical Model ===
/// - Quadcopters: N free-floating 6-DoF rigid bodies with thrust and torque
/// control.
/// - Payload: A spherical rigid body resting on the ground with friction.
/// - Ropes/Tethers: N bead-chains, each with tension-only spring-damper
/// segments.
///
/// === Control Strategy ===
/// Each quadcopter uses a cascaded controller:
/// 1. Position controller: PD control to track x/y/z trajectory with formation
/// offset.
...
///
/// All quadcopters follow the same trajectory with individual formation
/// offsets.
///
/// === Usage ===
///   ./quad_rope_lift
///
/// The simulation opens a Meshcat visualizer in your browser at
/// http://localhost:7000
...
struct QuadConfig {
  ...
  Eigen::Vector3d
      payload_attachment;  ///< Attachment point on payload surface [m]
  double rope_length_mean; ///< Expected rope rest length [m]
  double
      rope_length_stddev; ///< Standard deviation of rope length measurement [m]
...
int DoMain(int argc, char *argv[]) {
  ...
  int cli_fault_quad_index = -1; // 0-based index; -1 = no fault
  double cli_fault_time = -1.0;  // seconds; -1 = no fault
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (std::strcmp(argv[i], "--fault-quad-index") == 0 &&
               i + 1 < argc) {
      cli_fault_quad_index = std::atoi(argv[++i]);
    } else if (std::strcmp(argv[i], "--fault-time") == 0 && i + 1 < argc) {
      cli_fault_time = std::atof(argv[++i]);
    } else if (std::strcmp(argv[i], "--help") == 0) {
      std::cout << "Usage: ./quad_rope_lift [--seed N] [--headless] "
                   "[--duration T] [--num-quads N] [--output-dir DIR]\n"
                ...
                << "  --output-dir D        Base output directory (default: "
                   "../outputs)\n"
                << "  --fault-quad-index I  Quadcopter index (0-based) whose "
                   "cable to cut\n"
                << "  --fault-time T        Simulation time [s] at which to "
                   "cut the cable\n";
      return 0;
    }
  }
  ...
  // currently uses the basic QuadcopterLiftController (not the full GPAC
  // stack). Set to true once the GPAC layers (gpac_load_tracking_controller,
  ...
  const double formation_radius =
      0.6; // [m] - horizontal distance from payload center
  const double attachment_radius =
      payload_radius * 0.7; // Attachment points on payload top
  ...
  for (int i = 0; i < num_quadcopters; ++i) {
    ...
    std::normal_distribution<double> distribution(
        quad_configs[i].rope_length_mean, quad_configs[i].rope_length_stddev);
  ...
  // =========================================================================
  // Symmetric back-and-forth trajectory designed for fault isolation and
  // pre/post-fault performance comparison.
  //
  // Four equal cruise legs, each 16 s at 0.5 m/s along ±X:
  //   Leg 1 (+X): t= 7 → 23 s   (fault at t=15 = exactly midpoint)
  //   Leg 2 (-X): t=23.5→ 39.5 s  (first full post-fault leg — compare to Leg
  //   1) Leg 3 (-X): t=40 → 56 s Leg 4 (+X): t=56.5→ 72.5 s
  // Holds between legs are 0.5 s only (just enough to detect waypoint
  // crossing). Total duration: ~80 s.

  std::vector<TrajectoryWaypoint> waypoints;

  // Phase 1: Brief stabilize at ground — keep short so pickup is snappy
  waypoints.push_back({Eigen::Vector3d(0, 0, initial_altitude), 0.0, 1.0});

  // Phase 2: Ascend to cruise altitude (5 s transit, 1 s hold)
  waypoints.push_back({Eigen::Vector3d(0.0, 0.0, 3.0), 6.0, 1.0});

  // Leg 1: +X cruise, 8 m in 16 s = 0.5 m/s
  // Cable fault fires at t=15s → exactly the midpoint of this leg
  // Pre-fault segment: t=7→15 (8 s, 5-cable nominal)
  // Post-fault segment: t=15→23 (8 s, 4-cable degraded)
  waypoints.push_back({Eigen::Vector3d(8.0, 0.0, 3.0), 23.0, 0.5});

  // Leg 2: return −X, 8 m in 16 s = 0.5 m/s (first full post-fault leg)
  waypoints.push_back({Eigen::Vector3d(0.0, 0.0, 3.0), 39.5, 0.5});

  // Leg 3: −X extension, 8 m in 16 s = 0.5 m/s
  waypoints.push_back({Eigen::Vector3d(-8.0, 0.0, 3.0), 56.0, 0.5});

  // Leg 4: return +X, 8 m in 16 s = 0.5 m/s
  waypoints.push_back({Eigen::Vector3d(0.0, 0.0, 3.0), 72.5, 0.0});

  // Controlled descent
  waypoints.push_back({Eigen::Vector3d(0.0, 0.0, 2.0), 76.5, 0.0});
  ...
  for (int i = 0; i < num_quadcopters; ++i) {
    ...
    const std::vector<ModelInstanceIndex> visual_instances = parser.AddModels(
  ...
  for (int q = 0; q < num_quadcopters; ++q) {
    rope_params_vec[q] = ComputeRopeParameters(
        ...
        segment_stiffness, segment_damping,
  ...
  Multiplexer<double> *load_est_mux =
      nullptr; // combines quad 0's pos+vel into 6D

  if (enable_estimation) {
    std::cout << "\n========================================" << std::endl;
    std::cout << "State Estimation Enabled" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << "GPS sample rate: " << 1.0 / gps_sample_period << " Hz"
              << std::endl;
    std::cout << "GPS noise (x,y,z): " << gps_position_noise.transpose() << " m"
              << std::endl;
    std::cout << "Estimator rate: " << 1.0 / estimator_dt << " Hz" << std::endl;
    std::cout << "Use estimated in controller: "
              << (use_estimated_in_controller ? "YES" : "NO") << std::endl;
    std::cout << "========================================\n" << std::endl;
    ...
    load_gps_sensor =
        &(*builder.AddSystem<GpsSensor>(plant, payload_body, gps_params));
    builder.Connect(plant.get_state_output_port(),
    ...
    for (int q = 0; q < num_quadcopters; ++q) {
      ...
      auto &cable_len =
          *builder.AddSystem<ConstantVectorSource<double>>(len_vec);
  ...
  for (int q = 0; q < num_quadcopters; ++q) {
    ...
    auto &imu =
        *builder.AddSystem<ImuSensor>(plant, *quadcopter_bodies[q], imu_params);
  ...
  std::cout << "========================================" << std::endl;
  std::cout << "IMU sample rate: " << 1.0 / imu_params.sample_period_sec
            << " Hz" << std::endl;
  std::cout << "Gyro noise density: "
            << imu_params.gyro_noise_density.transpose() << " rad/s/sqrt(Hz)"
            << std::endl;
  std::cout << "Accel noise density: "
            << imu_params.accel_noise_density.transpose() << " m/s^2/sqrt(Hz)"
            << std::endl;
  std::cout << "========================================\n" << std::endl;
  ...
  std::cout << "========================================" << std::endl;
  std::cout << "Barometer sample rate: " << 1.0 / baro_params.sample_period_sec
            << " Hz" << std::endl;
  std::cout << "White noise stddev: " << baro_params.white_noise_stddev << " m"
            << std::endl;
  std::cout << "Correlated noise stddev: "
            << baro_params.correlated_noise_stddev << " m" << std::endl;
  std::cout << "========================================\n" << std::endl;
  ...
  std::cout << "========================================" << std::endl;
  std::cout << "Mean wind: " << wind_params.mean_wind.transpose() << " m/s"
            << std::endl;
  std::cout << "Turbulence (u,v,w): " << wind_params.sigma_u << ", "
            << wind_params.sigma_v << ", " << wind_params.sigma_w << " m/s"
            << std::endl;
  std::cout << "Gusts enabled: " << (gust_params.enabled ? "YES" : "NO")
            << std::endl;
  std::cout << "========================================\n" << std::endl;
  ...
  // Pointer to the safe hover controller for the fault quad (set below).
  ...
  for (int q = 0; q < num_quadcopters; ++q) {
    ...
    // If this is the fault quad, route rope forces through CableFaultGate
    ...
    // For the fault quad: after cable cut the quad retreats to a safe
    // hover position.  We add a SafeHoverController + ControlModeSwitcher
    // that transparently replace the normal controller output at fault_time.
    if (q == cli_fault_quad_index && cli_fault_time >= 0.0) {
      // Safe hover position: same radial direction as formation offset,
      // 2.5× formation radius outward (just outside the formation), at
      // z=5.0 m (2 m above cruise) so the freed quad clears all ropes and
      // the payload without drifting far from the scene.
      const double angle = 2.0 * M_PI * q / num_quadcopters;
      const double safe_r = formation_radius * 2.5;
      const Eigen::Vector3d safe_hover_pos(safe_r * std::cos(angle),
                                           safe_r * std::sin(angle), 5.0);

      fault_safe_ctrl = &(*builder.AddSystem<SafeHoverController>(
          plant, *quadcopter_bodies[q], safe_hover_pos, quadcopter_mass));
      builder.Connect(plant.get_state_output_port(),
    ...
    // Connect estimated state to controller (if estimation enabled and
    // configured)
  ...
  if (enable_estimation) {
    ...
    auto &load_error_computer =
        *builder.AddSystem<EstimationErrorComputer>(plant, payload_body);
    builder.Connect(plant.get_state_output_port(),
  ...
  {
    // Rope polyline visualizers intentionally omitted — bead spheres
    // rendered by MeshcatVisualizer are sufficient; explicit rope lines
    // are removed per research configuration.
    ...
    traj_vis_params.load_trail_color =
        drake::geometry::Rgba(1.0, 0.5, 0.0, 0.9);
    ...
    auto &trajectory_visualizer =
        *builder.AddSystem<tether_lift::TrajectoryVisualizer>(
            plant, meshcat, payload_body.index(), drone_body_indices,
            traj_vis_params);
    builder.Connect(plant.get_state_output_port(),
    ...
    // Hide the severed rope's bead chain in Meshcat at the fault time.
  ...
  logger_params.log_reference_trajectory =
      false; // No separate trajectory system yet
  ...
  logger_params.log_rope_states =
      false; // Need rope state output ports (future)
  ...
  for (int q = 0; q < num_quadcopters; ++q) {
    ...
    builder.Connect(imu_mux.get_output_port(0), data_logger.get_imu_input(q));
  }
  ...
  for (int q = 0; q < num_quadcopters; ++q) {
    ...
    if (q == cli_fault_quad_index && fault_safe_ctrl != nullptr) {
      fault_safe_ctrl->set_mass(quadcopter_mass + visual_model_mass);
    }
  }
  ...
  for (int q = 0; q < num_quadcopters; ++q) {
    ...
    const auto initial_bead_positions =
        GenerateSlackRopePositions(rope_start_world, rope_end_world,
                                   num_rope_beads, quad_configs[q].rope_length,
                                   0.85, // slack_ratio
                                   lateral,
  ...
  if (enable_estimation) {
    ...
    for (int q = 0; q < num_quadcopters; ++q) {
      auto &gps_context =
          quad_gps_sensors[q]->GetMyMutableContextFromRoot(&context);
      quad_gps_sensors[q]->InitializeGpsState(&gps_context,
                                              quad_configs[q].initial_position);
    }

    // Initialize load GPS sensor
    auto &load_gps_context =
        load_gps_sensor->GetMyMutableContextFromRoot(&context);
    ...
    for (int q = 0; q < num_quadcopters; ++q) {
      auto &est_context =
          quad_estimators[q]->GetMyMutableContextFromRoot(&context);
      quad_estimators[q]->SetInitialState(&est_context,
                                          quad_configs[q].initial_position,
                                          Eigen::Vector3d::Zero());
    }
    ...
    for (int q = 0; q < num_quadcopters; ++q) {
      auto &load_est_context =
          load_estimators[q]->GetMyMutableContextFromRoot(&context);
  ...
  //   1. quad_attachment_offset.z() = -(quad_height/2 + 0.05) ≈ -0.10m (below
  //   drone)
  //   2. rope_length (with stretch under load ~15%) ≈ rope * 1.15
  //   3. payload_attachment.z() = payload_radius ≈ 0.15m (above load center)
  //
  // So load_center_z = drone_z + quad_attachment_offset.z() - rope_length*1.15
  // - payload_radius
  ...
  const double total_vertical_offset =
      std::abs(quad_attachment_offset
                   .z()) + // Distance from drone center to rope attachment
  ...
  std::cout << "  quad_attachment_offset.z = " << quad_attachment_offset.z()
            << " m" << std::endl;
  ...
  std::cout << "  total_vertical_offset = " << total_vertical_offset << " m"
            << std::endl;
  ...
  // For subsequent waypoints, compute load position as quad position minus
  // total offset
  for (const auto &wp : waypoints) {
    // Load center position accounting for all offsets
    double load_z = wp.position.z() - total_vertical_offset;

    // Load can't go below ground (load center is at payload_radius above
    // ground)
  ...
  if (trajectory_visualizer_ptr) {
    trajectory_visualizer_ptr->DrawReferenceTrajectory(
        load_reference_waypoints);
  }
  ...
  config_params["use_estimated_in_controller"] =
      use_estimated_in_controller ? "true" : "false";
  ...
  // Enable MeshCat recording for video export (always)
  ...
  if (cli_fault_quad_index >= 0 && cli_fault_time >= 0.0) {
    std::cout << "  [FAULT] Cable cut: quad " << cli_fault_quad_index
              << " at t=" << cli_fault_time << "s\n";
    const double angle_f = 2.0 * M_PI * cli_fault_quad_index / num_quadcopters;
    const double safe_r_f = formation_radius * 2.5;
    std::cout << "  [FAULT] Quad " << cli_fault_quad_index
              << " retreats to hover at (" << safe_r_f * std::cos(angle_f)
              << ", " << safe_r_f * std::sin(angle_f) << ", 5.0) m\n";
  }
  if (!headless)
  ...
  if (enable_estimation && !quad_error_loggers.empty()) {
    ...
    if (load_error_logger != nullptr) {
      ...
      const double rms_pos_err =
          std::sqrt(pos_err_sum / load_error_data.cols());
