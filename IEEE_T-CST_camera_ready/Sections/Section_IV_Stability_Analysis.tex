...
The five lemmas verify the hybrid-systems hypotheses of~\cite{GoebelSanfeliceTeel2012} on $\Omega_\tau^{\mathrm{dwell}}$, certifying that the feed-forward identity $T_i^{\mathrm{ff}}=T_i$ supplies the bounded-jump mechanism for hybrid practical-ISS. The decay rates $\lambda$ and $\rho$ are fully explicit from the closed-loop Lyapunov matrices $P_v$, $P_{xy}$ of \cref{lem:altitude-iss,lem:horizontal-iss}. The jump bound $\chi(\Delta_f)$ is structurally explicit via \eqref{eq:chi-explicit}, but the weighting constant $\kappa_V$ that scales the tension Lyapunov channel depends on the relative magnitudes of the tracking and tension error states; its numerical calibration is deferred to follow-on work and does not affect the $\rho < 1$ certificate or the $\rho_\mathrm{ana} \approx 0.005$ computation.
...
The empirical defence of H1 and H3 uses the same pre-declared missions as the theorem's consequences, so the validation is partially \emph{post hoc}; \cref{tab:domain-audit} documents the margins. The gate bounds are set by bead-chain physics: $\tau_{\mathrm{slack,max}} = \SI{40}{\milli\second}$ is one quarter of the Tikhonov frozen window $\tau_{\mathrm{pend}}/(2\pi)$, and $\eta_{\max} = 2.5\%$ is a Rayleigh-turbulence budget. \Cref{thm:reduction} is independently validated against the bead-chain truth model in \cref{sec:VI-reduction}, providing an \emph{a~priori} check that the reduction error is absorbed into $\gamma_\eta$ of \cref{thm:hybrid-stability} without appeal to the simulated missions.
...
Five lemmas underpin \cref{thm:hybrid-stability}: tension cancellation on the altitude channel
(\cref{lem:tension-cancel}); exponential ISS for altitude and
horizontal channels through Lyapunov matrices $P_v$ and
$P_{xy}$ (\cref{lem:altitude-iss,lem:horizontal-iss});
anti-swing invariance to surviving cardinality
(\cref{lem:antiswing-invariant}); bounded Lyapunov jump
$\chi(\Delta_f)$ at each fault
(\cref{lem:jump-continuity}); and the dwell-cycle
contraction $\rho \in (0,1)$ composing jump and decay
(\cref{lem:dwell-decay}). The tension coordinates at a fault jump are compared on the
post-fault survivor set by restricting the pre-fault vector to the remaining cables; this convention is needed in \cref{lem:jump-continuity}.
