...
The five lemmas and main theorem certify the architectural claim of \cref{sec:Introduction}---that a single decentralized feed-forward identity supplies the bounded-jump mechanism required for hybrid practical-ISS on the declared admissibility domain---rather than constituting a stand-alone theoretical contribution. The hybrid-systems framework is from~\cite{GoebelSanfeliceTeel2012}; the practical-ISS composition tools are standard. The value added is verifying the framework's hypotheses on $\Omega_\tau^{\mathrm{dwell}}$ with closed-form $\rho$, $\lambda$, and $\chi(\Delta_f)$. Among the recovery-envelope constants, $\lambda$ and $\rho$ are fully explicit from $P_v$, $P_{xy}$ \eqref{eq:Pv-numeric}; the jump bound $\chi(\Delta_f)$ is explicit in structure with coefficient $\kappa_V$ bounded but not numerically calibrated here---its quantification is identified as follow-on work in \cref{sec:discussion-audit}.
...
partially \emph{post hoc}; the theorem provides a \emph{conditional}
guarantee, and \cref{tab:domain-audit} documents the margins.
Two measures mitigate the circularity. First, the gate bounds
are set by the physics of bead-chain re-tension:
$\tau_{\mathrm{slack,max}} = \SI{40}{\milli\second}$ is one quarter
of the Tikhonov frozen window $\tau_{\mathrm{pend}}/(2\pi)$, and
$\eta_{\max} = 2.5\%$ is a Rayleigh-turbulence budget.
Second, \cref{thm:reduction} is independently validated against
the bead-chain truth model in \cref{sec:VI-reduction},
providing an \emph{a~priori} check that the reduced model tracks
the full plant within $O(\delta + \eta_{\max})$ on
$\Omega_\tau^{\mathrm{dwell}}$; the stability analysis absorbs
the residual into $\gamma_\eta$ of \cref{thm:hybrid-stability}
without appeal to the simulated missions.
Design-time certification of H1 and H3 for unseen trajectory
families is identified as future work.
...
We state five lemmas that support the main theorem.
...
\begin{equation}
  ...
  \qquad
  P_{xy} = \begin{bmatrix} 1.283 & 0.0167 \\ 0.0167 & 0.0344 \end{bmatrix}.
  \label{eq:Pv-numeric}
\end{equation}
...
\begin{proof}
After tension cancellation (\cref{lem:tension-cancel}), each
scalar tracking error $e$ satisfies
$\ddot{e} + K_d \dot{e} + K_p e = \ddot{p}_{\mathrm{slot}}
+ r_i(t)$ with $|r_i| = O(\theta_{c,\max}^2 T_i/m_i) +
O(\delta)$ bounded uniformly on $\Omega_\tau^{\mathrm{dwell}}$
under H1; for altitude this is \eqref{eq:alt-error-ode} with
$r_i$ from \cref{lem:tension-cancel}.
\begin{equation}
  \ddot{e}_{p,z,i} + K_d^z \dot{e}_{p,z,i} + K_p^z e_{p,z,i}
    = \ddot{p}_{\mathrm{slot},z} + r_i(t).
  \label{eq:alt-error-ode}
\end{equation}
For each channel, the closed-loop matrix is Hurwitz ($A_m$:
eigenvalues $s_{1,2} = -12 \pm \sqrt{44}$; $A_{m,xy}$:
$s_{1,2}^{xy} = (-15\pm\sqrt{105})/2$), so the unique
$P \succ 0$ satisfying $A^\top P + PA = -I$ exists. The
$2\times2$ Lyapunov equation is solved algebraically via
$p_{12} = 1/(2K_p)$, yielding the matrices in
\eqref{eq:Pv-numeric}. Setting $\zeta = [e,\dot{e}]^\top$,
$V = \tfrac{1}{2}\zeta^\top P \zeta$, differentiating along
trajectories, and applying Young's inequality to the
cross-terms from $\ddot{p}_{\mathrm{slot}} + r_i$ gives the
ISS inequalities \eqref{eq:lyapunov-decay-alt}--\eqref{eq:lyapunov-decay-xy}
with decay rates $|\mathrm{Re}(s_1)|$ of each $A$; the ISS
gains are proportional to $\lambda_{\max}(P)/\lambda_{\min}(P)$
and the input magnitude. Independence from $N_s$ follows
because neither $A_m$ nor $A_{m,xy}$ depends on $|\mathcal{S}|$.
\end{proof}
...
Lyapunov-proxy measurements yield
$\hat\rho_{\mathrm{V4}} = 0.20$ and $\hat\rho_{\mathrm{V5}} = 0.045$.
The gap arises from the additive constant $c > 0$ in
\eqref{eq:dwell-contraction}: the empirical ratio satisfies
\begin{equation}
  \hat\rho_k
    \;\triangleq\;
...
\end{equation}
so $\hat\rho_k$ converges to $\rho_{\mathrm{ana}}$ only when
$c \ll V(t_k^{\star,-})$. The inequality is conservative;
$\hat\rho_k$ upper-bounds $\rho_{\mathrm{ana}}$ plus the
disturbance floor and is tight only in the zero-disturbance
limit. Under Dryden $\SI{4}{\metre\per\second}$ wind, the floor
$c_0/\alpha_{\min}$ constitutes a non-negligible fraction of
the pre-fault Lyapunov value; from \eqref{eq:c-def},
$c/V(t_k^{\star,-}) \approx 0.195$ for V4 and $\approx 0.040$
for V5. The improvement from V4 to V5 ($\hat\rho: 0.20 \to 0.045$)
confirms \eqref{eq:rho-hat-decomp}: with ten seconds of dwell,
$V(t_2^{\star,-})$ decays further, reducing the $c/V$ ratio.
In disturbance-free conditions, $c \to \rho\,\chi(\Delta_f)$
...
The ablation (\cref{sec:VI-D}) confirms that disabling $T_i^{\mathrm{ff}}=T_i$ increases peak sag by $3.6$--$4.0\times$ and produces larger post-fault Lyapunov jumps (\cref{fig:fault-zoom}), directly evidencing the jump-bound role of the identity vs.\ the contraction-rate role of $\alpha_{xy}$.
\end{remark}
