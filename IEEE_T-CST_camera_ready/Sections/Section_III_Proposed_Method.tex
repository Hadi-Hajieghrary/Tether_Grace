...
\end{align}
\paragraph{Feasibility.} The vertical interval in \eqref{eq:qp-vert} is non-empty whenever $f_{\min} < f_{\max}$; hover feasibility holds in all scenarios, and Doctrine~D1 is satisfied at the $0.10\%$ worst QP-transition fraction (\cref{tab:domain-audit}).
...
\paragraph{Euler-angle attitude PD and output wrench.} With body angular velocity $\vect\omega_i^{\mathcal{B}} = \mat R_i^{\top}\, \vect\omega_i^{\mathcal{W}}$, the body torques are
\begin{align}
  \tau_{i,x}
    &= K_p^{\mathrm{att}}\!\left(
