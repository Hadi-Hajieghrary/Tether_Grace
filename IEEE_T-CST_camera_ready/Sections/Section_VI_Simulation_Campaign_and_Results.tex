...
\begin{table*}[htbp]
  ...
  \begin{tabular}{llp{5.5cm}cp{5cm}}
    ...
    \midrule
    \multicolumn{5}{l}{\textit{Capability Demonstration and Admissibility Audit}} \\[\smallskipamount]
...
\begin{figure*}[!t]
  ...
  \begin{minipage}[t]{0.55\textwidth}
    \vspace{0pt}
    \centering
    \includegraphics[width=\linewidth]{Figures/fig_trajectory_3d_V4.png}
  \end{minipage}
  \hfill
  % ---------- Right: self-announcement chain ----------
  \begin{minipage}[t]{0.40\textwidth}
    \vspace{0pt}
    \centering

    \subfloat[Surviving rope tension $T_1(t)$.]{%
      \includegraphics[width=\linewidth]{Figures/fig_self_announcement_V4_a.png}%
      \label{fig:self-announcement-a}}\\[-0.8ex]

    \subfloat[Feed-forward command $T_1^{\mathrm{ff}}(t)$.]{%
      \includegraphics[width=\linewidth]{Figures/fig_self_announcement_V4_b.png}%
      \label{fig:self-announcement-b}}\\[-0.8ex]

    \subfloat[Commanded thrust $f_1(t)$.]{%
      \includegraphics[width=\linewidth]{Figures/fig_self_announcement_V4_c.png}%
      \label{fig:self-announcement-c}}\\[-0.8ex]

    \subfloat[Payload altitude error $e_{L,z}(t)$.]{%
      \includegraphics[width=\linewidth]{Figures/fig_self_announcement_V4_d.png}%
      \label{fig:self-announcement-d}}
  \end{minipage}

  \caption{V4 fault-response visualization. Left: dual-fault payload trajectory over $t \in [8,30]\,\si{\second}$, with the dashed lemniscate reference, fault events at $t_1{=}12$\,s and $t_2{=}17$\,s, and formation snapshots at
  $t\in\{10,15,20\}$\,s. Right: self-announcement causal chain for fault~1, where drone~0 is severed at $t=\SI{12}{\second}$. Teal solid denotes the baseline FF-on case, and orange dashed denotes the P2-A FF-off ablation. With the identity active,
  $T_1^{\mathrm{ff}}$ tracks $T_1$ at the next tick, thrust steps proportionally, and altitude error remains bounded. With the identity disabled, $T_1^{\mathrm{ff}}\!=\!0$ remains flat, the thrust step is absent, and altitude error grows by approximately
  $\SI{300}{\milli\metre}$.}
  \label{fig:v4-trajectory-and-self-announcement}
  \label{fig:trajectory-3d-V4}
