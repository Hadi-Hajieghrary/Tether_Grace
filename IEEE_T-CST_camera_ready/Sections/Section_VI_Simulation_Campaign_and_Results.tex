...
\paragraph{P2-D --- Formation reshape.} We sweep $T_{\mathrm{ref}} \in \{8, 10, 12\}~\si{\second}$ with and without reshape on V4 (\cref{tab:reshape-period-sweep}).

\begin{remark}[NF3: Reshape benefit zero across periods and aggressive-period probe]
\label{rem:reshape-null}
  The reshape supervisor targets the quasi-static-hover tension distribution. At the tested periods, centripetal acceleration $a_c = (2\pi/T_{\mathrm{ref}})^2 A \approx 0.82$--$3.29~\si{\metre\per\second\squared}$ continuously modulates the per-rope distribution, dominating the hover-equilibrium asymmetry the supervisor targets. The quasi-static assumption of \cref{thm:reshape} is therefore not binding at any tested period, including the aggressive $T_{\mathrm{ref}}=\SI{6}{\second}$ probe (E8), which a priori predicted a $\sim 25.8\%$ reduction. Measured peak tension with reshape on is $\SI{330.2}{\newton}$, identical to off: $0.0\%$ benefit. Pushing reshape into a binding regime would require a lower-bandwidth reference family or an extended objective accounting for the dynamic load. \Cref{thm:reshape,sec:theorem-preservation} preserve the envelope outside the binding regime.
\end{remark}
...
\begin{table}[htbp]
  \caption{Formation-reshape period sweep (P2-D, NF3). Peak post-fault rope tension with and without the reshape supervisor across four trajectory periods on V4, including the aggressive-period probe at $T_{\mathrm{ref}}=\SI{6}{\second}$ (E8).}
  \label{tab:reshape-period-sweep}
  \centering
  \small
  \renewcommand{\arraystretch}{1.05}
  \begin{tabular}{crr}
    \toprule
    $T_{\mathrm{ref}}$ (s) & Peak post-fault, reshape off (N) & Peak post-fault, reshape on (N) \\
    \midrule
    6  & 330.2 & 330.2 \\
    8  & 191.4 & 191.4 \\
    10 & 153.8 & 153.8 \\
