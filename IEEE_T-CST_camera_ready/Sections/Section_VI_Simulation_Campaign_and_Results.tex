We audit the controller claims through a pre-specified simulation campaign in Drake~\cite{drake,tether_grace_code}. The campaign addresses each contribution in order: the reduction-fidelity audit checks the taut-cable reduction against the bead-chain truth model (C1, \cref{sec:reduction-and-reference}); the fault-progression scenarios provide simulation evidence for the hybrid practical-ISS envelope (C2, \cref{sec:stability}); the ablation campaigns isolate the feed-forward identity and characterize each extension's operating domain (C3, \cref{sec:extensions}). All numerical claims are deterministic point estimates from a single fixed Dryden seed~42, selected as a reproducible stress realization (\cref{rem:reproducibility}); they should not be interpreted as population statistics over wind seeds. Seeded-campaign CIs are follow-up work.
...
\paragraph{Admissibility-domain audit (H1 and H3).} \Cref{thm:reduction} and \cref{thm:hybrid-stability} require three gates to hold on the demonstrated trajectory: H1a (no slack run $> \SI{40}{\milli\second}$), H1b (aggregate slack duty cycle $\le 2.5\%$), and H3 (QP active-set transition fraction $\le 1\%$). \Cref{tab:domain-audit} reports measurements for V1--V6: every variant passes with margin. Worst observed values are \SI{35.8}{\milli\second} (H1a; V2--V5, $\SI{4.2}{\milli\second}$ margin), $1.66\%$ (H1b; V6, $0.84$~pp margin), and $0.10\%$ (H3; V4--V6, $0.90$~pp margin). These measurements certify domain membership for the demonstrated mission family; they do not prove H1--H3 for all references, wind envelopes, or fault schedules. For a new mission, the same gate audit must be repeated before invoking the conditional certificate. V6's lower H1a ($\SI{31.4}{\milli\second}$) is consistent with the MPC layer damping high-frequency QP transitions.
...
\paragraph{Statistical scope.} V1--V6 and the ablation campaigns use a single fixed Dryden seed~42. The principal ($+x$) wind component achieves a $2\sigma$ exceedance within the $[8,12]\,\si{\second}$ pre-fault cruise window at this seed, making it a reproducible stress case for the tested trajectory. The reported RMSE, sag, contraction, and ablation-effect values are deterministic point estimates on this fixed realization; they should not be interpreted as population statistics over wind seeds. Replication across paired common-random-number (CRN) seed sets with bootstrap intervals on ablation effect sizes is left for follow-up work. Binary gate pass/fail statements in \cref{tab:domain-audit} are deterministic for the audited seed and trajectory.
...
We audit \cref{thm:hybrid-stability} against three scenarios of increasing severity: V3 (single fault), V4 (dual, $\Delta t = \SI{5}{\second}$), V5 (dual, $\Delta t = \SI{10}{\second}$). The dwell condition \eqref{eq:dwell-assumption} requires $\Delta t \ge \tau_{\mathrm{pend}} = \SI{2.24}{\second}$; the dual-fault scenarios meet it with $\SI{2.76}{\second}$ and $\SI{7.76}{\second}$ margin.

\paragraph{V3 --- single fault.} Drone~0 severs at $t = \SI{12}{\second}$ in cruise. The four survivors respond without fault signals, peer communication, or supervisory intervention. Peak 3-D error $\SI{0.337}{\metre}$, IAE $\SI{0.452}{\metre\second}$, max sag $\SI{85.1}{\milli\metre}$. The settle time is below the \SI{1}{\milli\second} tick resolution, logged as 0~s by quantization rather than exact convergence: the bounded Lyapunov jump of \cref{lem:jump-continuity} is small enough at the near-equal-sharing hover operating point that the tracking-error step falls below the \SI{1}{\milli\second} tick resolution.
...
\paragraph{V6 --- full stack.} V6 repeats V4 with MPC~$+L_1$+reshape active. First-fault sag is $\SI{69.9}{\milli\metre}$ ($-12\%$ vs V4), consistent with the stack preserving the envelope while delivering incremental benefit.
...
Disabling the feed-forward raises cruise RMSE by $34$--$39\%$ and inflates peak sag by $3.6$--$4.0\times$. The degradation appears even pre-fault: without the feed-forward, the outer PD must reject the quasi-static sag from payload weight and centripetal acceleration, raising the steady-state floor that $\gamma_\eta$ must absorb. Post-fault, the Lyapunov jump at each severance is larger without the identity (\cref{lem:jump-continuity}), and the inter-event contraction at rate $\alpha_{xy}$ must dissipate from a proportionally higher initial value; the transient therefore outlasts the near-tick recovery visible in the FF-on case and decays instead on the pendulum damping timescale. \Cref{tab:ff-ablation} summarises and \cref{fig:fault-zoom} shows the V4 time-series comparison.
...
\label{rem:reproducibility}
  V1--V6 use a fixed Dryden seed~42, producing bit-exact output across runs at identical Drake and library versions. Inter-variant RMSE differences are therefore attributable to controller configuration and fault schedule on this common realization. Post-hoc inspection shows the principal ($+x$) wind component achieves a $2\sigma$ exceedance within $[8,12]$~s, making the realization a reproducible stress case for the declared trajectory. The reported RMSE, sag, contraction, and ablation-effect values are deterministic point estimates on this fixed realization; they should not be interpreted as population statistics over wind seeds. A Monte Carlo sweep would answer a different question: population-average performance over wind realizations. The present campaign instead audits the mechanism and admissibility gates on one fixed reproducible stress realization.

  Ablations (P2-A through robustness sweep) are deterministic point estimates at seed~42; CRN-paired replication yielding bootstrap CIs is follow-up work. The seed manifest (\texttt{seed\_manifest.csv}: \texttt{seed\_wind}, \texttt{seed\_init}, \texttt{seed\_sensor}, \texttt{seed\_fault}, \texttt{seed\_solver}) is archived alongside simulation outputs at~\cite{tether_grace_code}. The repository\cite{tether_grace_code} contains: the Drake simulator source with pinned version (\texttt{WORKSPACE} file), the \texttt{DecentralizedLocalController} implementation, the ablation CLI flags (\texttt{--no-ff}, \texttt{--no-l1}, \texttt{--no-mpc}, \texttt{--no-reshape}), the Dryden seed manifest (\texttt{seed\_manifest.csv}), and the post-processing scripts that produced Tables~VIII--XII.

  \textbf{Scope of H1--H3 certification.} \Cref{tab:domain-audit} certifies domain membership for the demonstrated mission family only. For a new trajectory, reference profile, wind envelope, or fault schedule, the same gate audit must be repeated before invoking the conditional certificate. The theorem provides no bounds for trajectories that violate H1--H3.
\end{remark}
