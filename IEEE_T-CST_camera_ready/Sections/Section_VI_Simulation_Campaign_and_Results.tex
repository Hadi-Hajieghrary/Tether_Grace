We audit the controller claims through a pre-declared simulation campaign in Drake~\cite{drake,tether_grace_code}: the reduction-fidelity audit checks C1 (\cref{sec:reduction-and-reference}); the fault-progression scenarios provide evidence for the hybrid practical-ISS envelope (C2, \cref{sec:stability}); the ablation campaigns isolate the feed-forward identity and characterize each extension's domain (C3, \cref{sec:extensions}). All numerical claims are deterministic point estimates from a single fixed Dryden seed~42 (\cref{rem:reproducibility}).
...
\paragraph{Statistical scope.} All campaigns use fixed Dryden seed~42, whose principal ($+x$) wind component achieves a $2\sigma$ exceedance within $[8,12]\,\si{\second}$---a reproducible stress case. The reported RMSE, sag, contraction, and ablation-effect values are deterministic point estimates; they should not be interpreted as population statistics. CRN-paired replication with bootstrap CIs is follow-up work.
...
Three quantities are at play and must be kept distinct throughout this section:
...
\Cref{thm:reduction} bounds the shape-mode deviation $|T_i^{\mathrm{KV}} - T_i^{\mathrm{qs}}|$ via the Lyapunov energy argument. The empirical audit here measures only $\varepsilon_i = T_i^{\mathrm{KV}} - T_{\mathrm{eff}}$, which is easier to compute but is a \emph{different} (and structurally larger) quantity. Its role is to characterize how load-asymmetric the post-fault configuration is, not to verify the theorem bound.

\Cref{thm:hybrid-stability} requires that the lumped model tracks the bead-chain truth in the sense of \cref{thm:reduction}. The domain-gate layer (\cref{tab:domain-audit}) certifies that $\eta$, max slack-run, and QP transition fraction all satisfy the theorem's hypotheses with margin across all six variants; this is the primary certificate.
...
The degradation mechanism follows \cref{rem:passive-ft}: without the identity, the altitude channel carries an unrejected load increment, inflating both steady-state floor and post-fault Lyapunov jumps (\cref{lem:jump-continuity}); \cref{tab:ff-ablation,fig:fault-zoom} summarize the $34$--$39\%$ RMSE increase and $3.6$--$4.0\times$ peak-sag inflation.
