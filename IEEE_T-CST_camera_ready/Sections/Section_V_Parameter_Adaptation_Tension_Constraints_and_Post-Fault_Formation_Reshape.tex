...
\paragraph{Admissibility regime and out-of-scope conditions.} Within $\Omega = \Omega_\tau^{\mathrm{dwell}} \cap \Omega_{\mathrm{QP, active}}$ and under H1--H3, the full architecture satisfies \eqref{eq:iss-envelope-explicit} with the closed-form gains of \cref{thm:hybrid-stability} and the channel-wise reductions above. The analysis treats the reference waypoints as a time-varying ISS input; the result is practical stability with $\bar{e}_\infty \sim \SI{0.3}{\metre}$ under nominal operation (analytical $\SI{0.27}{\metre}$, measured 3-D RMSE $\SI{0.312}{\metre}$ on V1/V2) and $\rho^k \|\vect e_p(0)\|_P + \bar{e}_\infty$ after $k$ faults, with $\rho_{\mathrm{ana}} = \exp(-\alpha_{xy}\tau_{\mathrm{pend}}) \approx 0.005$ and empirical $\hat\rho_{\text{V4}}=0.20$, $\hat\rho_{\text{V5}}=0.045$ both well below unity (\cref{sec:VI-C,tab:stability-evidence}). The analytical-vs-empirical gap reflects $\rho_{\mathrm{ana}}$'s leading-order nature, which neglects equilibrium shift and finite-time integration. The identity $T_i^{\mathrm{ff}} = T_i$ alone delivers this; no detection or inter-drone communication is required.
