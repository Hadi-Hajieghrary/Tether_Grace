Three optional layers extend the baseline cascade of \cref{sec:proposed} and address residual concerns that the identity $T_i^{\mathrm{ff}}=T_i$ does not target: (i) parameter mismatch in $(m_L, k_{\mathrm{eff}})$, (ii) horizon-aware tension-ceiling enforcement, and (iii) post-fault hover-equilibrium asymmetry. Each layer enters the closed loop through a disjoint channel, preserves the hypotheses H1--H3 of \cref{thm:hybrid-stability}, and, on the demonstrated trajectory family, is empirically non-binding. This section states the role of each layer and the architectural invariant that keeps the main certificate \eqref{eq:iss-envelope-explicit} valid when any combination is enabled. Detailed adaptation laws, MPC formulations, reshape geometry, and full preservation arguments are reported in the supplementary material.

\begin{table}[htbp]
  \caption{Optional extension layers and their relationship to the main certificate. None of the three uses peer communication for control; only the reshape supervisor uses a local detection latch.}
  \label{tab:extensions-summary}
  \centering
  \small
  \renewcommand{\arraystretch}{1.1}
  \begin{tabular}{lcccc}
    \toprule
    Layer & Detection? & Comm.? & Theorem & Empirical role \\
    \midrule
    $L_1$ adaptive altitude
      & No & No & Preserved & Mass-mismatch margin \\
    MPC tension ceiling
      & No & No & Preserved & Non-binding (NF2) \\
    Reshape supervisor
      & Latch & $\lceil\log_2 N\rceil$ bits & Optional & Non-binding (NF3) \\
    \bottomrule
  \end{tabular}
\end{table}
...
The single-step QP \eqref{eq:qp} cannot foresee a tension spike ahead. When safe working load dominates, we replace it with an $N_p$-step receding-horizon MPC carrying a soft penalized tension-ceiling constraint ($\vect s\ge\vect 0$, weight $w_s=10^4$). On each tick the MPC propagates a ZOH discretization of the slot-tracking error and a chord-linearized tension prediction, then solves a single warm-started OSQP~\cite{stellato2020osqp} program; only the first input is applied. Slack penalization preserves unconditional feasibility (\cref{cor:mpc-recfeas}); under D1 and Lipschitz-in-parameter right-hand sides, slack activation is $O(\bar\varepsilon_q)$ in the parameter-mismatch radius $\bar\varepsilon_q=\|q-q^\star\|_\infty$ (\cref{prop:mpc-slack}). The MPC reduces to the baseline QP whenever the predicted tension stays clear of the ceiling. Canonical settings: $N_p=5$ ($10$ at evaluation), $\Delta t_{\mathrm{MPC}}=\SI{0.01}{\second}$, $T_{\max}=\SI{100}{\newton}$, $k_{\mathrm{eff}}=\SI{2778}{\newton\per\metre}$.
