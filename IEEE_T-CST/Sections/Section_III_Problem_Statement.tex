...
Damping is scaled to preserve cable-level damping across discretization: a reference cable damping $c_{\mathrm{cable}}$ is derived from a reference configuration ($n_b{=}8$, $c_{\mathrm{ref}} = 15$~N$\cdot$s/m, $k_{\mathrm{ref}} = 300$~N/m) and per-segment damping is $c_s = c_{\mathrm{cable}} \cdot (n_b{+}1)$, ensuring dampers-in-series equivalence. Cable lengths are deterministic for each scenario (e.g., $[1.00, 1.08, 1.16]$~m for $N{=}3$); Monte Carlo campaigns additionally randomize lengths by ${\pm}5\%$ from these baselines, producing up to $19\%$ asymmetry.
...
Sections~\ref{Section_IV:Fault-Tolerant_Control_Architecture}--\ref{Section_VI:Simulation_Results} analyze the reduced controller actually used in the full-Drake fault-tolerance runs: reference-acceleration and ESO feedforward are disabled, the cable-tension ramp is replaced by a constant gain, and the matrix gains are specialized to the uniform scalar gains above. The force decomposition is therefore restated in Section~\ref{Section_IV:Fault-Tolerant_Control_Architecture} in the form used for the reported fault-tolerance evidence.
...
Two Fault Scenarios Under N-Agnosticism is the fundamental problem: (A)~agent~$i$'s own cable breaks, and (B)~another agent's cable breaks. Scenario~A is trivial to detect and isolate; Scenario~B is the difficult case and the paper's focus.
...
Section~\ref{sec:payload_tracking} reports the resulting separation distances for all observed faults, and Fig.~\ref{fig:vehicle_dynamics_seven} provides representative tilt and thrust histories in the most demanding scenario. The escape policy is therefore presented as a conservative, validated maneuver rather than as an optimized trajectory.
...
\end{equation}
where $\Delta T_i^{(\mathrm{imm})} \approx 4.2$~N denotes the immediate tension jump measured on a surviving cable in the canonical $N{=}3$ full-Drake traces, and $\sigma_T \approx 3.0$~N denotes the corresponding pre-fault fluctuation level in that channel. The estimate is therefore empirical, drawn from the same scenario family used to motivate Scenario~B, rather than a closed-form sensor-noise bound.
...
The formal problem statement is as follows: Given the system on manifold~\eqref{eq:config_manifold}, the N-agnostic constraint (Section~\ref{sec:n_agnostic}), the cable-snap fault model (Section~\ref{sec:cable_snap}), the lumped uncertainty~\eqref{eq:lumped_dynamics} with disturbance sets (Definition~\ref{def:disturbance_sets_ps}), and the architectural incompatibility of detection-based FDIR (Scenario~\ref{sec:why_fdir_fails}), the paper addresses two problems.
