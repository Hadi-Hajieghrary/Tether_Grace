% Section V: Experimental Methodology and Evidence Sources

This paper distinguishes three levels of claim: \textit{implemented} modules that exist in the codebase, \textit{reproducible} results reproducible from canonical artifacts, and \textit{open} hypotheses whose formal proof is incomplete (see Table~\ref{tab:claim_status} for a summary). The full-Drake simulations use the GPAC hierarchy's PID-plus-feedforward control law (Section~\ref{sec:layer1_control}) with PID feedback, gravity feedforward, and cable-tension feedforward as the fault-tolerance mechanism. The ESO feedforward and adaptive estimation are disabled. The reduced-order comparisons retain the CL baseline as the standardized reference.
