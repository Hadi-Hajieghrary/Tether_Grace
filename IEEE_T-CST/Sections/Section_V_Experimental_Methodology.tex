...
For the controller-comparison study, the \emph{centralized oracle} is intentionally narrow: at the fault time it replaces local cable-tension feedforward with perfect post-fault load-share feedforward on each surviving agent. It does \emph{not} reconfigure formation geometry, reset integral states, or modify the reference trajectory. The oracle should therefore be read as a feedforward-information baseline, not as a globally re-optimized centralized controller.
...
The evidence is organized in two tiers: (i)~ideal-condition runs (plant state, no wind) establishing the core mechanism and scalability, and (ii)~sensor-realistic runs (ESKF + Dryden wind) and Monte Carlo campaigns establishing robustness. Both tiers report RMSE and peak error ($\mathcal{L}_\infty$) to enable comparison with the theoretical bound (Proposition~\ref{thm:topology_invariance}). The broadened Monte Carlo study is smaller ($5$ runs) and is interpreted accordingly as exploratory evidence rather than as the same statistical tier as the $30$-run campaigns.
