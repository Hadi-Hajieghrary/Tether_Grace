...
The key finding is that payload RMSE \emph{decreases} with team size despite absorbing proportionally more faults: the $N{=}7$ scenario absorbs three cascading faults yet achieves $11\%$ lower RMSE than $N{=}3$ with one fault. Peak errors ($107$--$117$~cm) settle within $1$--$2$~s (Figs.~\ref{fig:tracking_errors_all},~\ref{fig:load_error_overlay}), and each snap produces a bounded reconfiguration rather than loss of transport (Fig.~\ref{fig:scenario_seven}).
...
To contextualize the passive GPAC approach, three alternative controllers are evaluated on the $N{=}3$ single-fault scenario (Table~\ref{tab:baseline_comparison}): a centralized oracle with perfect post-fault load-share knowledge, a gain-scheduled PID triggered by elevated cable tension, and a threshold-triggered reactive controller that boosts thrust by $+30\%$. The centralized oracle performs comparably to passive GPAC ($49.18$ vs.\ $48.84$~cm, $+0.7\%$), confirming that local cable sensing already captures nearly all fault-relevant information. All detection-based alternatives fall within $1.2\%$ of the passive mechanism, providing no meaningful benefit over the N-agnostic PID-plus-feedforward structure.

\begin{table}[t]
\centering
\caption{Controller comparison ($N{=}3$, single fault).}
...
The bead-chain cable model discretizes each continuous cable into $n_b$ point-mass beads connected by $(n_b{+}1)$ tension-only spring-damper segments. All preceding results use $n_b{=}8$. To assess whether the spatial resolution of the cable model materially affects the load-tracking conclusions, a discretization sweep varies $n_b \in \{2, 4, 8, 12, 16\}$ in the $N{=}5$ cascading-fault scenario (cable~1 snap at $7.0$~s, cable~3 snap at $12.0$~s) while holding all other parameters---cable rest length, total rope mass, payload mass, trajectory, fault schedule, controller gains, and simulation timestep ($\Delta t = 0.2$~ms)---fixed.

\emph{Invariant preservation.}
Cable-level stiffness is preserved exactly: the runner computes per-segment stiffness as $k_{\mathrm{seg}} = k_{\mathrm{cable}} \cdot (n_b{+}1)$, correct for springs in series. Cable-level damping is held invariant by deriving a reference cable damping from the $n_b{=}8$ baseline and scaling per-segment damping as $c_{\mathrm{seg}} = c_{\mathrm{cable}} \cdot (n_b{+}1)$. The collision-geometry bead radius is clamped at $12$~mm for all tested bead counts; consequently, bead contact forces operate under a fixed geometric cap rather than scaling with segment length.

\emph{Results.}
Table~\ref{tab:bead_discretization} reports the event-centered metrics. The first-fault and second-fault peak errors and recovery times are evaluated using clean time windows that exclude startup transients and avoid cross-contamination between fault events. The whole-run RMSE in this table is computed from $t \geq 3.0$~s (excluding liftoff), which differs from the full-horizon RMSE in Table~\ref{tab:scenario_summary}.

% Table generated from outputs/bead_count_sweep/five_drones/bead_sweep_metrics.json
\begin{table}[t]
  \centering
  \caption{Cable discretization sensitivity ($N{=}5$).}
  \label{tab:bead_discretization}
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{r c c c c c c}
    \toprule
    $n_b$ & \makecell{Fault\,1\\peak [cm]} & \makecell{Fault\,1\\$t_{\mathrm{rec}}$ [s]}
     & \makecell{Fault\,2\\peak [cm]} & \makecell{Fault\,2\\$t_{\mathrm{rec}}$ [s]}
     & \makecell{Whole-run\\RMSE [cm]} & \makecell{Wall\\time [s]} \\
    \midrule
...
Fig.~\ref{fig:bead_discretization} overlays the load tracking error for $n_b \in \{4, 8, 16\}$. Across the full discretization range, the first-fault peak error varies by only $0.3$~cm ($31.5$--$31.8$~cm), the second-fault peak error by $0.2$~cm ($43.2$--$43.4$~cm), and the whole-run RMSE by $0.1$~cm ($43.1$--$43.2$~cm). Recovery times are effectively identical at $4.09$~s (fault~1) and $3.67$~s (fault~2) for all bead counts. The metrics converge between $n_b{=}4$ and $n_b{=}8$, with changes from $8$ to $16$ beads below $0.15$~cm in all scalar metrics.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{./Figures/bead_discretization/bead_discretization_composite.pdf}
  \caption{Load tracking error under cable discretization sweep ($N{=}5$, cascading faults). (a)~Full horizon; (b)~first-fault zoom ($t{=}7$~s); (c)~second-fault zoom ($t{=}12$~s). Dashed red lines mark fault times.}
  \label{fig:bead_discretization}
\end{figure}

\emph{Interpretation.}
The load-tracking response is effectively insensitive to cable discretization over the tested range ($n_b{=}2$ to $16$), with all scalar metrics varying by less than $1\%$. The main fault-tolerant transport conclusion---that the N-agnostic architecture absorbs cable topology changes without reconfiguration---is preserved across the tested discretization range. The $n_b{=}8$ baseline used throughout this paper captures the essential fault dynamics; finer discretization does not alter the mechanism, the scalability trend, or the quantitative metrics to within the precision reported in the preceding tables. Runtime scales approximately as $\mathcal{O}(n_b^{1.5})$, making the $n_b{=}8$ choice a practical tradeoff between resolution and computational cost.

% ============================================================
\subsection{Summary of Evidence}
% ============================================================

The full-Drake simulation results establish:

\begin{enumerate}[label=(\roman*),nosep]
  \item Stable payload tracking through abrupt cable-snap faults across $N \in \{3, 5, 7\}$, with load RMSE $44$--$49$~cm decreasing with team size (Table~\ref{tab:scenario_summary}). Empirical $\rho_{\mathrm{FT}}$ values ($1.26$--$1.39$) are consistent with the transient-aware prediction.
  \item Under matched single-fault conditions, RMSE decreases monotonically from $48.8$ to $38.4$~cm as $N$ increases from $3$ to $7$ (Table~\ref{tab:matched_single_fault}). Fault-count isolation at $N{=}7$ (Table~\ref{tab:fault_count_isolation}) deconfounds the remaining variable.
  \item A sharp thrust-saturation phase transition at $f_{\max} \approx 45$~N coincides with the theoretical post-fault minimum; the default $150$~N provides $3\times$ headroom (Table~\ref{tab:fmax_boundary}).
  \item Sensor-realistic evaluation (Table~\ref{tab:sensor_realistic}), Monte Carlo validation (Table~\ref{tab:monte_carlo}), supplementary trajectory-generalization experiments, and baseline comparison (Table~\ref{tab:baseline_comparison}) confirm robustness across conditions.
  \item ESO-active evaluation (Table~\ref{tab:eso_evaluation}) confirms that $\rho_{\mathrm{FT}}$ increases modestly with a stronger controller while absolute post-fault RMSE decreases, demonstrating that fault tolerance is not an artifact of a deliberately weakened controller.
  \item Cable discretization sensitivity (Table~\ref{tab:bead_discretization}) confirms that varying the bead-chain resolution from $n_b{=}2$ to $16$ changes all scalar load-tracking metrics by less than $1\%$, establishing that the preceding results are not artifacts of a particular spatial discretization.
\end{enumerate}

Complementary reduced-order studies are provided in the supplementary material. The central conclusion is that N-agnostic decentralized architectures absorb cable topology changes without explicit fault detection or reconfiguration, as explained by the topology-independent error structure (Proposition~\ref{thm:topology_invariance}).
