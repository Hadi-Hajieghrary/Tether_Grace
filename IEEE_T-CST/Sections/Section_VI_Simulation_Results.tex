...
This section reports full-Drake multibody simulation results across $N \in \{3, 5, 7\}$ with abrupt cable-snap faults. All results use the Drake~\cite{drake2024} plant at $\Delta t_{\mathrm{sim}} = 0.2$~ms with bead-chain cable dynamics and $30$~s figure-eight trajectories.
...
% ============================================================
\subsection{Deconfounding: Team Size, Fault Count, and Thrust Margin}
...
\begin{table}[t]
\centering
\caption{Fault-count isolation: $N{=}7$ with $k \in \{1, 2, 3\}$ faults.}
...
\midrule
\multicolumn{1}{r}{$\Delta$} & $-0.44$ & $+0.06$ & $+0.77$ \\
\bottomrule
\end{tabular}
\end{table}
...
\end{equation}
where $f_{\mathrm{track}} \approx m_Q \cdot a_{\max} \approx 8$~N accounts for figure-eight tracking demand (matching the observed pre-fault thrust above hover). The default $f_{\max} = 150$~N provides $3\times$ headroom, confirming that the claimed fault tolerance does not depend on aggressive actuator sizing.
...
\begin{table}[t]
\centering
\caption{ESO-active evaluation: $\rho_{\mathrm{FT}}$ sensitivity to controller complexity.}
...
\begin{table}[t]
\centering
\caption{Sensor-realistic evaluation: ESKF + Dryden wind active.}
\label{tab:sensor_realistic}
\begin{tabular}{lccccc}
\toprule
Scenario & \multicolumn{2}{c}{RMSE [cm]} & \multicolumn{2}{c}{Peak error [cm]} & $\Delta_{\mathrm{RMSE}}$ \\
         & Ideal & ESKF+wind & Ideal & ESKF+wind & [\%] \\
\midrule
$N{=}3$, 1~fault   & 48.84 & 53.68 & 107.44 & 119.14 & $+9.9$ \\
$N{=}5$, 2~faults  & 46.37 & 50.14 & 112.30 & 125.17 & $+8.1$ \\
$N{=}7$, 3~faults  & 43.60 & 48.28 & 117.45 & 131.56 & $+10.7$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[label=(\roman*),nosep]
  ...
  \item Sensor-realistic evaluation (Table~\ref{tab:sensor_realistic}), Monte Carlo validation (Table~\ref{tab:monte_carlo}), trajectory generalization (Table~\ref{tab:trajectory_generalization}), and baseline comparison (Table~\ref{tab:baseline_comparison}) confirm robustness across conditions.
