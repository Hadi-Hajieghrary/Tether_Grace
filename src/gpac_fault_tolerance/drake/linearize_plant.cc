...
/// @brief Standalone tool for H∞ small-gain verification (reviewer M5).
///
...
//  separate library. For now, this file documents the approach.)
...
int main(int argc, char* argv[]) {
  ...
  // and the main runner call. For now, this file serves as a skeleton.
  //
  // The plant includes:
  ...
  std::cout << "=== LINEARIZATION APPROACH ===\n\n"
            << "1. Build MultibodyPlant with " << num_quads << " quads + payload + cables\n"
            << "2. Simulate " << hover_duration << "s in hover to reach taut equilibrium\n"
  ...
  for (int k = 0; k < n_freq; ++k) {
    ...
    Eigen::Vector3cd G_vec = sI_minus_A.inverse() * B.cast<std::complex<double>>();
  ...
  std::cout << "Predicted peak error (Δd=4.4): "
            << gamma_FT * 4.4 << " m = " << gamma_FT * 4.4 * 100 << " cm\n";
  ...
  for (int i = 0; i < 3; ++i) eigs_vec(i) = eigs(i);
  ...
  info << "{\n"
       ...
       << "  \"note\": \"Full-plant linearization requires refactoring plant-building code into shared library. Scalar model verification is complete.\"\n"
       << "}\n";
  ...
  std::cout << "\nThe cable-dynamics gain γ_cable depends on cable stiffness,\n"
            ...
            << " provides\nthe per-axis position-loop gain for the small-gain check.\n";
